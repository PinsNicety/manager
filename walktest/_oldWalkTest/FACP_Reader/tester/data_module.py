...
class DataCenter(TextReader, ExcelWriter):
    ...
    def __init__(self, panel_type):
        ...
        """
        # self.BASE_PATH = 'D:\\projects\\WalkTest\\FACP_Reader\\tester\\static\\'
        self.BASE_PATH = os.getcwd()
        self.excel_sheet = os.path.join(self.BASE_PATH, 'static/walktest.xlsx')

        if panel_type == "2-3030":
            self.data_file = os.path.join(self.BASE_PATH, 'static/2-3030/Springfield Bulk Mail_2-3030_Device List.txt')
            self.test_file = os.path.join(self.BASE_PATH, 'static/2-3030/Springfield Bulk Mail_2-3030_Test History.txt')
        elif panel_type == "640":
            self.data_file = os.path.join(self.BASE_PATH, 'static/Zanetti_device list.txt')
            self.test_file = os.path.join(self.BASE_PATH, 'static/Zanetti_test history.txt')
        elif panel_type == '320':
            self.data_file = os.path.join(self.BASE_PATH, 'static/320/Plymouth Airport Admin_Devices.txt')
            self.test_file = os.path.join(self.BASE_PATH, 'static/320/Plymouth Airport Admin_Test.txt')
